...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verifyCsrfToken($csrfToken)) {
        ...
        try {
            ...
            setFlash('success', 'Customer ' . e($customer['customer_code']) . ' details updated successfully.');
            redirect('modules/customers/view.php?id=' . $id);
        } catch (Exception $e) {
            error_log('Customer Update Error: ' . $e->getMessage());
            $error = 'An error occurred while updating customer details.';
        }
    }
}
?>
...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
  <div>
    <h4 class="fw-bold text-dark m-0">Edit Customer</h4>
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb m-0 small">
        ...
        <li class="breadcrumb-item"><a href="<?= BASE_URL; ?>modules/customers/view.php?id=<?= $id; ?>" class="text-decoration-none"><?= e($customer['customer_code']); ?></a></li>
        <li class="breadcrumb-item active" aria-current="page">Edit</li>
      </ol>
    </nav>
  </div>
  <div class="d-flex gap-2">
    <a href="<?= BASE_URL; ?>modules/customers/view.php?id=<?= $id; ?>" class="btn btn-outline-secondary d-inline-flex align-items-center gap-1">
    ...
    <div class="card shadow-sm">
      <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
        <h6 class="m-0 fw-semibold text-dark">
          <i class="bi bi-pencil-square me-2 text-primary"></i> Edit Customer Information
        </h6>
        <span class="badge bg-light text-dark border font-monospace">
          ID: <?= e($customer['customer_code']); ?>
        </span>
      </div>
      ...
      <div class="card-body p-4">
        ...
        <form method="POST" action="<?= BASE_URL; ?>modules/customers/edit.php?id=<?= $id; ?>" novalidate>
          <?= csrfField(); ?>

          <!-- Read-only Summary Strip -->
          <div class="p-3 bg-light rounded border mb-4 d-flex flex-wrap gap-4 small text-muted">
            <div>
              <span class="fw-semibold text-dark">Customer Code:</span>
              <span class="font-monospace text-primary fw-bold ms-1"><?= e($customer['customer_code']); ?></span>
            </div>
            <div>
              <span class="fw-semibold text-dark">Registered Date:</span>
              <span class="ms-1"><?= date('M d, Y h:i A', strtotime($customer['created_at'])); ?></span>
            </div>
            <div>
              <span class="fw-semibold text-dark">Last Modified:</span>
              <span class="ms-1"><?= date('M d, Y h:i A', strtotime($customer['updated_at'])); ?></span>
            </div>
          </div>
          ...
          <div class="mb-4">
            ...
            <div class="row g-3">
              <div class="col-md-6">
                ...
                <textarea class="form-control" id="address" name="address" rows="3"><?= e($address); ?></textarea>
              </div>
          ...
          <div class="d-flex justify-content-end gap-2 pt-3 border-top">
            <a href="<?= BASE_URL; ?>modules/customers/view.php?id=<?= $id; ?>" class="btn btn-outline-secondary px-4">Cancel</a>
